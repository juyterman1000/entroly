...
def _hdr(txt, icon=""):
    ...
    print(f"\n{S.BG_BL}{S.WH}{S.B} {'─' * left} {icon} {txt} {'─' * right} {S.R}")
...
def _m(lab, val, c=S.WH, ind=4):
    print(f"{' ' * ind}{S.GY}{lab:<38}{S.R}{c}{val}{S.R}")
...
def act1_the_pain():
    ...
    print(
        f"  ║{S.R}{S.BG_DK}  {S.CY}{S.B}  E N T R O L Y  —  Developer Experience Demo            {S.WH}{S.BG_DK}   {S.WH}║"
    )
    print(
        f"  ║{S.R}{S.BG_DK}  {S.GY}  Real Rust engine • Real metrics • No fakes                 {S.WH}{S.BG_DK} {S.WH}║"
    )
    print(
        f"  ║{S.R}{S.BG_DK}                                                                      {S.WH}║"
    )
...
""")
    ...
    for src, f in naive_dropped:
        ...
        print(
            f"    {tag}{S.R}  {S.GY}{src:<28}{S.R} {S.D}{f['tokens']:>3} tok (no room){S.R}"
        )
...
""")
    ...
    try:
        ...
        print(
            f"  {S.RD}✗ entroly_core not available — run: cd entroly-core && maturin develop{S.R}"
        )
        return False
...
def act3_real_engine():
    ...
    for src, f in CODEBASE.items():
        ...
        print(
            f"    {icon}{S.R}  {S.GY}{src:<28}{S.R}  {S.D}{detail}{S.R}  {S.D}({elapsed_us:.0f}µs){S.R}"
        )
    ...
    for s in selected:
        ...
        print(
            f"    {tag}{S.R}  {S.WH}{src:<28}{S.R}  {S.D}{tokens:>3} tok{S.R}  entropy {ebar}  relevance {rbar}"
        )
    ...
    entroly_f1 = (
        ...
        * entroly_precision
        * entroly_recall
        / max(entroly_precision + entroly_recall, 1e-9)
    )
    ...
    _m(
        ...
        f"{dupes_caught} ({tokens_saved_dedup} tokens saved)",
        S.CY,
    )
...
""")
    ...
    try:
        ...
        print(
            f"    {S.GY}  recall={baseline['avg_recall']:.4f}  precision={baseline['avg_precision']:.4f}  "
...
def act6_business_value(
    ...
    token_waste_pct_before = (
        (total_tokens - sum(f["tokens"] for f in CODEBASE.values() if f["relevant"]))
        / total_tokens
        * 100
    )
    token_waste_pct_after = (
        (tokens_used - sum(f["tokens"] for f in CODEBASE.values() if f["relevant"]))
        / max(tokens_used, 1)
    ) * 100

    entroly_recall - naive_recall
    entroly_precision - naive_precision
    ...
    time_saved_per_task_min = (
        reprompts_before - reprompts_after
    ) * 2  # 2 min per re-prompt cycle
...
""")
    ...
    internals = [
        ...
        (
            ...
            f"Caught {dupes} near-duplicate(s) via hamming distance — saved {tokens_saved} tokens",
        ),
        (
            "🎯",
            "Hybrid Semantic Matching",
            "SimHash + n-gram Jaccard for query-aware relevance ranking",
        ),
        (
            "🎒",
            "0/1 Knapsack DP",
            f"Maximized value within {TOKEN_BUDGET}-token budget in {opt_ms:.2f}ms",
        ),
        (
            "🧠",
            "Ebbinghaus Decay",
            "Older fragments naturally deprioritized — recent work stays fresh",
        ),
        (
            "🔗",
            "Import Dependency Graph",
            "Auto-links related files via import/identifier analysis",
        ),
        (
            "⚖️",
            "Compare-Calibrate Filter",
            "Post-selection redundancy check — swaps similar fragments",
        ),
        (
            "🎲",
            "ε-Greedy Exploration",
            "Explores new fragments occasionally to prevent feedback starvation",
        ),
        (
            "🛡️",
            "SAST Security Scan",
            "Auto-flags hardcoded secrets, SQL injection, unsafe patterns",
        ),
        (
            "💀",
            "Skeleton Substitution",
            "Fits structural summaries of excluded files into remaining budget",
        ),
    ]
...
def main():
    ...
    # Act 3: Real engine
    engine, e_recall, e_precision, e_f1, opt_ms, dupes, tokens_saved, tokens_used = (
        act3_real_engine()
    )
