...
code = re.sub(
    r"compress_pruner\(context, question, args\.budget, method\)",
    "compress_agentic_pruner(context, question, args.budget, method)",
    code,
)
code = re.sub(r"args\.pruner_method", '"agentic_pruning"', code)
code = re.sub(
    r'--pruner-method.*?help=.*?TokenPruner comparison"\)',
    '--skip-pruner", action="store_true", help="skip agentic pruner")',
    code,
    flags=re.DOTALL,
)
...
for cell in nb.get("cells", []):
    if "source" in cell and len(cell["source"]) > 0:
        if (
            "Load Token Pruner" in cell["source"][0]
            or "Load LLMLingua" in cell["source"][0]
        ):
            cell["source"] = [
                "## 7. Initialize Agentic Context Pruner (2026 SOTA Baseline)\n"
            ]
        elif (
            "from llmlingua import PromptCompressor" in cell["source"][1]
