...
| Single installed MCP surface | The normal `entroly` MCP server registers the same nine Work Graph continuity tools as the focused `entroly-work-graph-mcp` entrypoint through one shared adapter. Repository-scoped attachment grants include the `continuity` scope by default, so `entroly attach create ...` exposes resume, handoff, verified context compilation/page faults and the observable Work/Context/Trust event loop without requiring a second MCP configuration. |
| Release artifact metadata | Both Python package manifests explicitly emit Core Metadata 2.4 for wheels and sdists. This prevents allowed Hatchling 1.32 builds from defaulting to Metadata 2.5, which the required Twine 6.2 validation rejects. Root and nested 1.0.79 wheel/sdist pairs build and pass `twine check`. |
...
| Primary MCP convergence at `aa2c8544` | Focused MCP/attachment/CLI regression: `54 passed`. Full source run: `3968 passed, 161 skipped, 3 xfailed`; its three failures all invoked the ambient 1.0.78 console launcher. An isolated exact-head 1.0.79 wheel reran those exact console/MCP cases: `3 passed`. Root and nested wheel+sdist artifacts subsequently passed Twine after the explicit Metadata 2.4 fix. |
