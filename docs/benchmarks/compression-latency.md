...
than the published External Baseline A 0.31.0 wheel for warm public compression calls
...
| Fixture | Entroly warm p50 | External Baseline A warm p50 | Speedup | Entroly cold p50 | External Baseline A cold p50 | Speedup |
|---|---:|---:|---:|---:|---:|---:|
...
External Baseline A median latency to Entroly median latency. The confidence interval
...
- External Baseline A entry point: `External Baseline A.compress`, documented `agent-90` profile,
...
- External Baseline A environment: exact `External Baseline A-ai==0.31.0` wheel; `pip check` reported
