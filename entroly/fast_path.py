...
class FastPathRouter:
    ...
    def _load_skills(self) -> list[_SkillEntry]:
        ...
        for info in entries:
            ...
            # Parse those static values without importing or executing the
            # writable vault module.
    ...
    def _extract_from_tool(
        ...
        Parse static generated assignments without executing module code.
        """
        ...
        try:
            ...
            logger.debug("skill %s tool.py parse failed: %s", skill_id, e)
            return [], None, {}
        ...
        for node in tree.body:
            ...
            if not isinstance(target, ast.Name):
                continue
            if target.id in {"FRAGMENT_RECIPE", "WEIGHT_PROFILE"}:
                try:
                    values[target.id] = ast.literal_eval(node.value)
                except (ValueError, TypeError):
                    continue
            elif target.id == "TRIGGER_PATTERN":
                pattern = FastPathRouter._parse_trigger_pattern(node.value)

        recipe = values.get("FRAGMENT_RECIPE") or []
        if not isinstance(recipe, list):
            recipe = []
        recipe = [str(x) for x in recipe if x]

        weights = values.get("WEIGHT_PROFILE") or {}
    ...
    @staticmethod
    def _parse_trigger_pattern(node: ast.AST) -> re.Pattern[str] | None:
        """Parse a generated ``re.compile(pattern, re.I)`` call safely."""
        ...
        # Generated triggers are literal alternations wrapped in word
        # boundaries. Reject quantifiers so edited vault files cannot install
        # catastrophic-backtracking patterns on the request hot path.
        unescaped = re.sub(r"\\.", "", raw)
        if any(ch in unescaped for ch in "*+?{"):
            return None
