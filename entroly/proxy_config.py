...
class ProxyConfig:
    ...
    # Runs AFTER WITNESS as the final hallucination guard. Fully deterministic
    # local computation — no neural model, no LLM calls. Configurable
    # independent of WITNESS. See benchmarks/results/ for accuracy numbers
    # on the public test sets evaluated (FEVER, SQuAD v2, HaluEval-QA).
    #   off       → disabled
    #   audit     → analyze every response; emit X-Entroly-EICV-* headers only (DEFAULT)
    #   annotate  → keep body, append "[Entroly EICV] verification warnings"
    #   strict    → graduated 4-action rewrite of hallucinated claims
    # profile: rag | qa | summarization | dialogue | fact_check | default
    # Note: false-positive and false-negative rates are non-zero; see
    # benchmarks/results/fever_esg.json for our measured operating points.
    eicv_mode: str = "audit"
    eicv_profile: str = "rag"
    eicv_embed: bool = False
