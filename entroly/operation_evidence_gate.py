...
class OperationEvidenceGate:
    ...
    def autonomous_execution_allowed(self) -> bool:
        """Caller-supplied lexical evidence cannot authorize execution."""
        return False
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        result["preflight_passed"] = self.preflight_passed
...
def build_operation_evidence_gate(
    *,
    ...
    ``pass`` means only that these local checks found no contradiction. It
    does not authorize execution or verify the answer's factual assertions.
    """
    if not task.strip():
