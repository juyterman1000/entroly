...
impl NkbeAllocator {
    ...
    pub fn allocate(&mut self) -> serde_json::Value {
        ...
        if n == 1 {
            ...
            let mut agent_dict = serde_json::Map::new();
            agent_dict.insert("budget".into(), self.global_budget.into());
            agent_dict.insert("weight".into(), agent.weight.into());
            agent_dict.insert("utility".into(), self.agent_utility(0, 0.0).into());
            result.insert(agent.name.clone(), serde_json::Value::Object(agent_dict));
            return serde_json::Value::Object(result);
        }
        ...
        if total_min >= self.global_budget {
            for agent in &self.agents {
                ...
                d.insert("budget".into(), agent.min_budget.into());
                d.insert("weight".into(), agent.weight.into());
                d.insert("utility".into(), serde_json::Value::from(0.0_f64));
                result.insert(agent.name.clone(), serde_json::Value::Object(d));
            }
            return serde_json::Value::Object(result);
        }
        ...
        for i in 0..n {
            let mut d = serde_json::Map::new();
            d.insert("budget".into(), budgets[i].into());
            d.insert("weight".into(), self.agents[i].weight.into());
            d.insert("utility".into(), utilities[i].into());
            result.insert(self.agents[i].name.clone(), serde_json::Value::Object(d));
        }
    ...
    pub fn stats(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "last_lambda_star": self.last_lambda_star,
            "last_dual_gap": self.last_dual_gap,
            "tau": self.tau,
        })
    }
}
