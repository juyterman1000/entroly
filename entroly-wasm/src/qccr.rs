//! QCCR — Query-Conditioned Compressive Retrieval: file-ranking core (WASM).
//!
//! Byte-for-byte the same ranking logic as `entroly-core/src/qccr.rs` (the PyO3
//! build), so npm and pip/MCP/SDK rank identically. The only differences are the
//! bindings (wasm-bindgen instead of PyO3) and `regex-lite` instead of `regex`
//! to keep the WASM bundle lean. See the core module for the full rationale.
//!
//! Model: score(file) = w_bm25 · normalize(BM25F(body, path)) + Σ_k w_k · feature_k
//! Weight DEFAULTS live here (SSOT); JS passes only per-repo overrides.
...
/// Rank files by `w_bm25 · normalize(BM25F) + Σ feature_k` (see core module).
pub fn rank_files(
    sources: &[String],
    texts: &[String],
    query: &str,
...
// ── wasm-bindgen bindings (JSON in/out — JS does JSON.parse) ─────────────────
/// Rank files; returns a JSON array of `[index, score]` pairs, best-first.
/// `overrides_json` is a JSON object of per-repo weight overrides (or "{}").
#[wasm_bindgen]
pub fn qccr_rank_files(
    ...
    let ranked = rank_files(&sources, &texts, query, &ov);
