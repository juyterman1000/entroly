...
def main():
    ...
    check(
        ...
        lambda: f"rec={w0['w_recency']:.3f}, sem={w0['w_semantic']:.3f}",
    )

    # Verify weights sum to ~1
    check(
        "Weights sum to 1",
        lambda: f"sum={sum(w0.values()):.4f}"
    ...
    check(
        "Recency upweighted after positive signal",
    ...
    check("Negative advantage reduces recency", lambda: f"rec={w_neg['w_recency']:.3f}")

    # --- Learning rate decay ---
    check(
        "LR decays with observations",
    ...
    for t in threads:
        t.join()
    check(
        "Thread-safe (4×100 concurrent updates)",
    ...
    check(
        "Query bonus",
        lambda: f"with={r4:.3f}, without={r5:.3f}"
        if r4 >= r5
        else (_ for _ in ()).throw(Exception("no bonus")),
    )
    ...
    state = prism.state()
    check(
        "State export", lambda: f"n={state.n_observations}, alphas={len(state.alphas)}"
    )
    ...
    check(
        "Engine has OnlinePrism", lambda: f"type={type(engine._online_prism).__name__}"
    )
    ...
    check(
        "optimize_context succeeded",
        lambda: f"selected={len(result.get('selected_fragments', result.get('selected', [])))}",
    )

    prism_info = result.get("online_prism", {})
    check(
        "OnlinePrism in result",
        lambda: f"reward={prism_info.get('reward')}, "
    ...
    check(
        "After 6 calls",
        lambda: f"n={prism_stats['n_observations']}, "
    ...
    check(
        "Weights have adapted",
        lambda: f"total_drift={total_drift:.4f}, "
        f"weights={', '.join(f'{d}={w_evolved[d]:.3f}' for d in WEIGHT_DIMS)}",
    )

    print("\n" + "=" * 60)
    print(
        f"  PASSED: {passed}/{passed + failed}  |  FAILED: {failed}/{passed + failed}"
    )
    if failed == 0:
