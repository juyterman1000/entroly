...
def _benchmarked_engine(tmp_path):
    ...
    result = engine.benchmark_skill(created['skill_id'])
    assert result['fitness'] == 1.0, result
    return engine, created['skill_id'], folder


@pytest.mark.parametrize('mutation', ['tool', 'cases', 'fitness', 'trust', 'receipt',
                                     'evaluator', 'invalid_count', 'invalid_fitness', 'shape'])
def test_real_benchmark_promotion_rejects_changed_inputs(tmp_path, mutation):
    ...
    if mutation == 'tool':
        ...
        if mutation == 'fitness':
            metrics['fitness_score'] = .99
        elif mutation == 'trust':
            metrics['benchmark_evidence_class'] = 'untrusted'
        elif mutation == 'evaluator':
            metrics['benchmark_binding']['evaluator_sha256'] = 'changed'
        elif mutation == 'invalid_count':
            metrics['benchmark_runs'] = 'not a number'
        elif mutation == 'invalid_fitness':
            metrics['fitness_score'] = 'not a number'
        elif mutation == 'shape':
            metrics = []
        else:
            metrics.pop('benchmark_binding')
    ...
    assert result['binding_blocked'] is True
