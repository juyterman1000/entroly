...
from entroly.verifiers.stave import stave_risk

ds = load_dataset("pminervini/HaluEval", "qa_samples", split="data")
items = [(str(r["knowledge"]), str(r["question"]),
          str(r["answer"]), 1 if r["hallucination"] == "yes" else 0)
         for r in ds if r.get("knowledge") and r.get("answer")]
random.Random(SEED).shuffle(items)
items = items[:N]

analyzer = WitnessAnalyzer(use_nli=False, force_python=True, profile="benchmark_qa")

_ENT_RE = [re.compile(r'\b\d+\.?\d*\b'),
...
print(f"  STAVE coverage: {n_signal}/{n_dec} decisions have relational signal ({100*coverage:.1f}%)\n")
print(f"  {'System':<30}  {'AUROC':>7}  {'Best-Acc':>9}")
print(f"  {'-'*52}")
print(f"  {'WITNESS alone':<30}  {auroc_w:>7.4f}  {100*acc_w:>8.1f}%")
print(f"  {'WITNESS + entity-gap (baseline)':<30}  {auroc_fus:>7.4f}  {100*acc_fus:>8.1f}%")
print(f"  {'STAVE alone':<30}  {auroc_stave:>7.4f}  {100*acc_stave:>8.1f}%")
print(f"  {'WITNESS + entity-gap + STAVE':<30}  {auroc_sfus:>7.4f}  {100*acc_sfus:>8.1f}%")
