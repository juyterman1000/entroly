...
TIER1_TARGETS = {
    "halueval_qa": {
        "metric": "auroc",
        "target": 0.85,
        "anchor": "DeBERTa-v3-large NLI ~0.85",
    },
    "halueval_dialogue": {
        "metric": "f1",
        "target": 0.68,
        "anchor": "NLI baselines ~0.78",
    },
    "halueval_summarization": {
        "metric": "f1",
        "target": 0.55,
        "anchor": "NLI baselines ~0.65",
    },
    "truthfulqa": {"metric": "auroc", "target": 0.78, "anchor": "LLM-judge baselines"},
    "factscore": {
        "metric": "atomic_precision",
        "target": 0.72,
        "anchor": "GPT-4 0.76 / smaller 0.60-0.65",
    },
    "fever": {
        "metric": "accuracy",
        "target": 0.75,
        "anchor": "DeBERTa-large 0.74 / LLM 0.78",
    },
    "true": {"metric": "mean_auroc", "target": 0.80, "anchor": "DeBERTa-v3-large 0.79"},
    "hhem": {
        "metric": "leaderboard_rank",
        "target": "top_decile",
        "anchor": "leaderboard-relative",
    },
    "ragas": {
        "metric": "faithfulness_auroc",
        "target": "report_only",
        "anchor": "RAGAS uses LLM-judge, no fair comparison",
    },
}

ROBUSTNESS_TARGETS = {
    "falsification_survived_auroc_drop": {
        "target_max": 0.03,
        "anchor": "Fusion-4 collapse 0.441 on HaluEval-QA",
    },
    "worst_manifold_ece": {
        "target_max": 0.08,
        "anchor": "LLM-judge ~0.15, DeBERTa-NLI ~0.12",
    },
    "conformal_coverage_violation_a01": {
        "target_max": 0.02,
        "anchor": "Theoretical bound under exchangeability",
    },
...
def _adapt_witness_slices(raw: list | None, source_hash: str | None) -> dict:
    ...
    if raw is None or not isinstance(raw, list):
        return {
            "status": "not_run",
            "reason": "witness_benchmarks.json missing or unexpected shape",
        }
    ...
    return {
        ...
        "halueval_dialogue_n200": _adapt_one(
            _slice("HaluEval-Dialogue (string/local)")
        ),
        "halueval_summarization_n200": _adapt_one(
            _slice("HaluEval-Summarization (string/local)")
        ),
    }
...
def _adapt_ragas(raw: dict | None, source_hash: str | None) -> dict:
    if raw is None:
        return {
            "status": "not_run",
            "reason": "ragas_faithfulness_benchmark.json missing",
        }
    w = raw.get("witness", {})
    f = raw.get("fusion", {})
    return {
...
def _adapt_falsification_legacy(
    raw: dict | None, source_hash: str | None, *, source_file: str, applies_to: str
) -> dict:
    ...
    fusion_aurocs = [
        c.get("fusion") for c in raw.get("conditions", []) if isinstance(c, dict)
    ]
    ...
    return {
        ...
        "conditions": {
            "C1_original": {
                "fusion_auroc": _g("C1", "fusion"),
                "witness_only": _g("C1", "witness_only"),
                "g_only": _g("C1", "g_only"),
            },
            "C2_entity_ctrl": {
                "fusion_auroc": _g("C2", "fusion"),
                "witness_only": _g("C2", "witness_only"),
                "g_only": _g("C2", "g_only"),
            },
            "C3_paraphrase": {
                "fusion_auroc": _g("C3", "fusion"),
                "witness_only": _g("C3", "witness_only"),
                "g_only": _g("C3", "g_only"),
            },
            "C4_realistic": {
                "fusion_auroc": _g("C4", "fusion"),
                "witness_only": _g("C4", "witness_only"),
                "g_only": _g("C4", "g_only"),
            },
        },
...
def _adapt_simple_baseline(
    raw_and_hash: tuple[dict | None, str | None], *, default_reason: str
) -> dict:
    ...
    return {
        ...
        "preregistered_target": (
            raw.get("preregistered_target_accuracy") or raw.get("preregistered_target")
        ),
...
def _trust_classify(per_benchmark: dict[str, dict], probes: dict[str, dict]) -> dict:
...
def build_baseline() -> dict:
    ...
    fals_sum_raw, fals_sum_hash = _read_json(
        "halueval_summarization_falsification.json"
    )
    ...
    per_benchmark = {
        ...
        if slices_payload.get("status") == "ok"
        else {"status": "not_run"},
        "halueval_summarization": {
            "status": "ok_stale_n200",
        ...
        "factscore": _adapt_simple_baseline(
            _read_json("factscore_baseline.json"),
            default_reason="adapter_pending_wikipedia_fetcher",
        ),
        "fever": _adapt_simple_baseline(
            _read_json("fever_baseline.json"),
            default_reason="fever_baseline_not_yet_run",
        ),
        "true": _adapt_simple_baseline(
            _read_json("true_baseline.json"),
            default_reason="adapter_pending_11_subtasks",
        ),
        "hhem": _adapt_simple_baseline(
            _read_json("hhem_submission.json"),
...
def main() -> int:
    ...
    for item in v["headline_only"]:
        ...
        if item.get("min_auroc_c1_c4") is not None:
            ...
            print(
                f"  [FAIL] {b}: falsification did NOT survive — "
    ...
    print()
    print(
        "Primary metric (Risk@Coverage worst-manifold): "
        "deferred to Phase 4 (EICV components not yet landed)"
    )
    print()
