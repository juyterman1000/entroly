...
mod tests {
    ...
    fn test_bm25_relevance_ordering() {
        ...
        // Score decomposition invariant (coverage-amplified BM25):
        //   combined = bm25_base * (1 + 1.5 * coverage^3) + path_boost + identifier_boost
        // This guards against silent regressions in the explainability breakdown.
        for s in [&s1, &s2, &s3] {
            let expected = s.bm25_base * (1.0 + 1.5 * s.query_coverage.powi(3))
                + s.path_boost
                + s.identifier_boost;
            assert!(
