...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted maps, caller/program graphs, structural health, and two-phase rename plans carry source hashes, ambiguity, omissions, and tamper-evident receipts. <sub>An operator-configured LSP can add bounded workspace references through framed JSON-RPC; Entroly verifies every returned range but cannot enforce the external server's network behavior. Rename apply requires the preview hash and incompleteness acknowledgement, validates staged syntax, and attempts rollback. It does not claim compiler-complete references; [design and limitations](docs/verified-code-context.md).</sub>
