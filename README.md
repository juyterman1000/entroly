...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted repository maps, task context, caller graphs, Python type-informed dispatch and control/data flow, value-free runtime overlays, and structural health audits carry source hashes, explicit ambiguity, stale-index rejection, omissions, and tamper-evident receipts. <sub>Opt-in content-addressed persistence reuses exact parse, whole-graph, map, and health artifacts across unchanged trees and independently rechecks their commitments. Health findings are review aids, not defect proofs; grammar and semantic coverage vary; [design and limitations](docs/verified-code-context.md).</sub>
