...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted repository maps, task context, caller graphs, Python type-informed dispatch/control flow, runtime overlays, structural health, and two-phase rename plans carry source hashes, explicit ambiguity, omissions, and tamper-evident receipts. <sub>Rename apply requires the exact preview hash and an explicit incompleteness acknowledgement, then rechecks preimages, validates staged syntax, and attempts rollback on write failure. It does not claim compiler-complete references. Opt-in persistence reuses unchanged verified analyses; [design and limitations](docs/verified-code-context.md).</sub>
