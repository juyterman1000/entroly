...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. A task query becomes a budgeted partial code graph with source hashes, explicit ambiguity, stale-index rejection, omissions, and a tamper-evident receipt. <sub>Grammar coverage varies; cross-file and type-directed binding remains conservative; no surprise parser downloads; [design and limitations](docs/verified-code-context.md).</sub>
