...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted repository maps, task context, caller graphs, Python type-informed dispatch and control/data flow, and value-free runtime overlays carry source hashes, explicit ambiguity, stale-index rejection, omissions, and tamper-evident receipts. <sub>Typed personalized PageRank prioritizes repository anchors; opt-in content-addressed parsing reuses unchanged files; grammar and semantic coverage vary; no surprise parser downloads; [design and limitations](docs/verified-code-context.md).</sub>
