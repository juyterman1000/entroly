...
- **Understands code and command outcomes** — an optional local-first parser pack provides exact declaration spans across 27+ language grammars, while shell profiles preserve invocations, failures, and terminal summaries under tight budgets. <sub>No surprise parser downloads; unknown languages and commands retain deterministic fallbacks and exact recovery.</sub>
