...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted repository maps, task context, caller graphs, Python type-informed dispatch and control/data flow, value-free runtime overlays, and structural health audits carry source hashes, explicit ambiguity, stale-index rejection, omissions, and tamper-evident receipts. <sub>Health audits expose parser-derived complexity and nesting, import cycles, coupling, language coverage, and unresolved-call risk under a published scoring policy; they are review aids, not proofs of defects. Grammar and semantic coverage vary; no surprise parser downloads; [design and limitations](docs/verified-code-context.md).</sub>
