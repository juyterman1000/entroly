...
- **Builds verified code context** — an optional local-first parser pack maps 27+ language grammars and emits exact declaration and recognized call-site spans. Budgeted maps, typed graph queries, architecture layers/communities/routes, snapshot diffs, program flow, structural health, and two-phase rename plans carry source hashes, ambiguity, omissions, and tamper-evident receipts. <sub>An operator-configured LSP can add bounded workspace references; Entroly verifies returned ranges but cannot enforce the external server's network behavior. Community and hotspot scores are disclosed heuristics, and rename does not claim compiler-complete references; [design and limitations](docs/verified-code-context.md).</sub>
